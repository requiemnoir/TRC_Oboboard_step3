...
def parse_vag_mirror_payload(payload: bytes):
    ...
    def _try_at(off):
        ...
        if ntype == 1:
            ...
            if end > len(payload):
                return None
            return (10 + dlc, bus_ch, raw_id, payload[off + 10 : end], "CAN")
        elif ntype == 0:
            # FlexRay frame: id = [SlotID:1][CycleCount:1]
            if dlc > 254:
                return None
            end = off + 10 + dlc
            if end > len(payload):
            ...
            if slot_id == 0:
                return None  # slot 0 not valid
            return (10 + dlc, bus_ch, slot_id, payload[off + 10 : end], "FlexRay")
        else:
            return None
    ...
    for start in [4, 3, 2, 0]:
        ...
        while i + 10 <= len(payload):
            ...
            if hit is None:
                i += 1
                continue
            consumed, bus_ch, frame_id, data, net_type = hit
            frames.append((bus_ch, frame_id, bytes(data), net_type))
            if net_type == "FlexRay":
                score += 2
            else:
                score += 1
...
def main():
    ...
    ap.add_argument("--sample", default=os.path.join(ROOT_DIR, "tests", "data", "vag_mirror_single_payload.bin"),
                    help="Path to raw SOME/IP mirror payload (default: golden sample)")
    ap.add_argument("--repeat", type=int, default=500, help="Number of times to repeat the payload (simulate packets)")
    ...
    for db in databases:
        for msg in db.messages:
            fid = msg.frame_id
            if fid not in decode_map:
                decode_map[fid] = (msg, db)
    print(f"  CAN decode map: {len(decode_map)} messages from {len(databases)} DBC(s)")
    ...
    # --- 2. Load & parse sample payload ---
    with open(args.sample, "rb") as fh:
        raw = fh.read()
    # Strip SOME/IP header if present
    if len(raw) >= 20:
    ...
    template_frames = parse_vag_mirror_payload(inner)
    if not template_frames:
        print("ERROR: no frames parsed from sample payload")
        sys.exit(1)

    can_count = sum(1 for f in template_frames if f[3] == "CAN")
    fr_count = sum(1 for f in template_frames if f[3] == "FlexRay")
    print(f"  Parsed {len(template_frames)} frames per payload packet ({can_count} CAN, {fr_count} FlexRay)")
    ...
    for rep in range(args.repeat):
        ...
        for idx, (bus_ch, frame_id, data, net_type) in enumerate(template_frames):
            ...
            if net_type == "CAN":
                ...
                # DBC decode
                msg_entry = decode_map.get(arb_id)
                ...
                msg, db = msg_entry
                try:
                    decoded = msg.decode(bytes(data) if len(data) >= msg.length else bytes(data) + b'\x00' * (msg.length - len(data)))
                except Exception:
                    total_raw += 1
                    continue
    ...
    total_frames = args.repeat * len(template_frames)
