...
from xcp_can_client import XcpCanClient, default_xcp_can_config, DEFAULT_GEARBOX_SIGNALS
...
def _get_xcp_can_client(autocreate: bool = True) -> Optional['XcpCanClient']:
    ...
    with _xcp_can_lock:
        if _xcp_can_client is None and autocreate:
            ...
            stored = (config_store.get_config_only() or {}).get('xcp_can')
    ...
    try:
        ...
        for bus_num in can_buses:
            ...
            if 99 not in new_map:
                new_map[99] = phys
            # AUTOSAR mirror channel 100+net_id → phys
            new_map[100 + phys] = phys
...
def mf4_get_decoded_data():
    ...
    if (not dbc_name) and (not dbc_names) and (not auto):
        return jsonify({'ok': False, 'error': 'missing dbc'}), 400
    if not isinstance(signals, list) or not signals:
        return jsonify({'ok': False, 'error': 'signals must be a non-empty list'}), 400
    ...
    try:
        ...
        try:
            ...
            for sess_id, sess_name in strategies:
                ...
                # Tester Present (suppress response usually 0x80, but here let's ensure connectivity with response)
                _ = scanner._uds_transact(int(target_addr), bytes([0x3E, 0x00]), timeout_s=0.5)
                time.sleep(0.05)

                # Pre-condition: DTC Off (Best effort)
                try:
                    # 0x85 0x02 = ControlDTCSetting Off.
                    scanner._uds_transact(int(target_addr), bytes([0x85, 0x02]), timeout_s=0.5)
                except Exception:
                    pass
                ...
                # Try SA (blindly) if requested OR if we detected a lock.
                # However, without a key algo, we can't do much. 
                # We'll skip SA unless configured.

                # ------- Security Access (0x27) -------
                # NRC 0x24 (requestSequenceError) typically means SA is required.
                # Try common SA levels. We attempt a zero-key first (development ECUs),
                # then XOR-based seed→key (common in some VAG units).
                def _try_security_access(sa_level: int) -> bool:
                    """Attempt Security Access at the given odd level (e.g. 0x01, 0x11, 0x03, 0x61)."""
                    seed_resp = scanner._uds_transact(
                        int(target_addr),
                        bytes([0x27, int(sa_level) & 0xFF]),
                        timeout_s=1.5,
                    )
                    if not seed_resp or seed_resp[0] != 0x67:
                        return False
                    seed = seed_resp[2:]  # skip [67] [level]
                    ...
                    key_candidates = [
                        seed,                                                     # echo (dev)
                        bytes([b ^ 0xFF for b in seed]),                           # complement
                        bytes([(b + 1) & 0xFF for b in seed]),                     # +1
                        bytes(len(seed)),                                          # zeros
                        bytes([0xFF] * len(seed)),                                 # all-FF
                    ]
                    send_level = (int(sa_level) + 1) & 0xFF
                    for key in key_candidates:
                        key_resp = scanner._uds_transact(
                            int(target_addr),
                            bytes([0x27, send_level]) + key,
                            timeout_s=1.5,
                        )
                        if key_resp and key_resp[0] == 0x67:
                            print(f"Security Access level 0x{sa_level:02X} unlocked (key method)")
                            return True
                        # If NRC 0x36 (exceededNumberOfAttempts) or 0x37 (requiredTimeDelayNotExpired), stop
                        if key_resp and len(key_resp) >= 3 and key_resp[0] == 0x7F and key_resp[2] in (0x36, 0x37):
                            print(f"SA level 0x{sa_level:02X}: NRC 0x{key_resp[2]:02X}, aborting SA attempts")
                            return False
                        # Re-request seed for next attempt (ECU invalidates previous seed after wrong key)
                        seed_resp2 = scanner._uds_transact(
                            int(target_addr),
                            bytes([0x27, int(sa_level) & 0xFF]),
                            timeout_s=1.5,
                        )
                        if seed_resp2 and seed_resp2[0] == 0x67:
                            seed = seed_resp2[2:]
                            if not seed or all(b == 0 for b in seed):
                                return True  # already unlocked
                        else:
                            break  # can't get seed
                    return False

                sa_done = False
                for sa_lvl in [0x01, 0x11, 0x03, 0x61]:
                    if _try_security_access(sa_lvl):
                        sa_done = True
                        break
                if sa_done:
                    time.sleep(0.1)
                ...
                # -----------------------------------------------------
                # NEW: Ensure Development Messages (0x0902) are ENABLED
                # This master switch is required on R4 software to allow
                # mirror traffic (UDP) to flow. Default is often 0x00.
                # -----------------------------------------------------
                try:
                    # Check current value
                    _val0902 = scanner._uds_transact(int(target_addr), bytes([0x22, 0x09, 0x02]), timeout_s=0.5)
                    # Expected: 62 09 02 <val>
                    if _val0902 and len(_val0902) >= 4 and _val0902[0] == 0x62:
                        ...
                        # Blind write if read fails or yields weird response (some ECUs require SA for Read?)
                        print("[Mirror] Forcing Dev Messages (0x0902) to 0x01 (blind)")
                        scanner._uds_transact(int(target_addr), bytes([0x2E, 0x09, 0x02, 0x01]), timeout_s=0.5)
                except Exception as e:
                    print(f"Warning: Failed to set Development Messages (0x0902): {e}")

                # Pre-condition: Start mirror activation routine (0x0203)
                # On VAG MLBevo gateways, RoutineControl 0x0203 is related to
                # mirror/network-test activation and may be required before DID
                # writes succeed.
                try:
                    rc_resp = scanner._uds_transact(
                        int(target_addr),
                        bytes([0x31, 0x01, 0x02, 0x03]),
                        timeout_s=1.5,
                    )
                    if rc_resp and rc_resp[0] == 0x71:
                        print(f"RoutineControl 0x0203 started OK")
                        time.sleep(0.15)
                except Exception:
                    pass
