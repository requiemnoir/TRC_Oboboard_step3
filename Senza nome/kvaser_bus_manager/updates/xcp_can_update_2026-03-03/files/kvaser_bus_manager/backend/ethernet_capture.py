...
class EthernetCapture:
    ...
    def _try_unpack_vag_mirror(self, payload):
        ...
        def _try_at(off: int):
            ...
            # Bus channel is a 1-byte value in this layout candidate.
            # Real systems typically use <= 8 buses; allow some slack.
            if bus_ch > 15:
                return None

            # NetworkType: 1=CAN, 0=status/bus-state block.
            # Status blocks can contain arbitrary bytes that may look like CAN records;
            # when they are well-formed, consume them to avoid false positives.
            if ntype == 0:
                # Heuristic: status blocks in this layout commonly have reserved==0 and can_id==0.
                # Treat dlc as a payload length when it is plausible.
                if reserved != b"\x00\x00" or can_id != 0x0000:
                    return None
                if dlc > 512:
                    return None
                end = off + 10 + dlc
                if end > len(payload):
                    return None
                return (10 + dlc, "STATUS", None, None)

            if ntype != 1:
                return None

            # Classic CAN payload should be <= 8 bytes, but some captures include padding.
            # Keep an upper bound to avoid garbage while not being overly strict.
            if dlc > 64:
                return None
            end = off + 10 + dlc
            if end > len(payload):
            ...
            return (10 + dlc, "CAN", bus_ch, can_id, data)
        ...
        for start in start_offsets:
            ...
            while i + 10 <= len(payload):
                ...
                rec_type = hit[1]
                if rec_type == "CAN":
                    bus_ch, arb_id, data = hit[2], hit[3], hit[4]
