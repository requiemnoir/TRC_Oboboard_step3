...
try:
    flush_mb = (_saved_config or {}).get('mf4_flush_interval_mb')
    if flush_mb is None:
        flush_mb = float(os.getenv('MF4_FLUSH_INTERVAL_MB', '5') or 5)
    shared_logger.set_mf4_flush_interval_mb(flush_mb)
except Exception:
    pass
...
# ── Mirror UDP Listener — bridge live-data dal mirror_logger su :30490 ───────
# Avvia un socket UDP che ascolta i pacchetti AUTOSAR ISO 23150 (e fallback)
# PARALLELO al mirror_logger.MirrorCapture (AF_PACKET). Su Linux i due tipi
# di socket ricevono entrambi senza interferenza; il KBM decodifica via
# DBC/ARXML/FIBEX e inietta i frame nel BusManager con capture_origin='mirror'.
# La UI Live Traffic mostra i frame con badge "mirror".
#
# Attivazione (off di default per non rompere setup esistenti):
#   KBSM_MIRROR_LISTEN_ENABLED=1
#   KBSM_MIRROR_LISTEN_PORT=30490        (default)
#   KBSM_MIRROR_LISTEN_HOST=0.0.0.0      (default; '127.0.0.1' per loopback-only)
try:
    from mirror_udp_listener import maybe_start as _maybe_start_mirror_listener
    _mirror_udp_listener = _maybe_start_mirror_listener(
        manager,
        config_resolver=lambda: config_store.get_config_only() or {},
    )
except Exception as _e:
    _mirror_udp_listener = None
...
def api_mirror_listener_stats():
    ...
    Ritorna pacchetti UDP ricevuti, frame iniettati, errori, pps, fps.
    Se il listener è disabilitato (KBSM_MIRROR_LISTEN_ENABLED!=1) restituisce
    {ok: true, enabled: false}.
    """
    if _mirror_udp_listener is None:
        return jsonify({'ok': True, 'enabled': False,
                        'reason': 'KBSM_MIRROR_LISTEN_ENABLED diverso da 1, o init fallita'})
    try:
        return jsonify({'ok': True, 'enabled': True, 'stats': _mirror_udp_listener.stats()})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
def _experimental_defaults() -> dict:
    return {
        ...
        'logs_retention_max_age_days': 21,
        'logs_retention_max_total_mb': 8192,
        'logs_retention_min_interval_s': 300.0,
        'lamp_mappings': {
            'mil': {'message': '', 'signal': ''},
            'epc': {'message': '', 'signal': ''},
...
def api_health():
    ...
    min_free_mb = float(os.getenv('KBSM_MIN_FREE_DISK_MB', '256') or 256)
    try:
        ...
        health['disk'] = {
            ...
            'low': bool(free_mb > 0 and free_mb < min_free_mb),
        }
    except Exception:
        try:
            ...
            health['disk'] = {
                ...
                'used_bytes': int(usage.used),
                'free_bytes': int(usage.free),
                'free_mb': round(float(usage.free) / (1024.0 * 1024.0), 1),
                'min_free_mb': min_free_mb,
                'low': False,
            }
        except Exception:
...
@app.route('/api/maintenance/enforce_retention', methods=['POST'])
def api_maintenance_enforce_retention():
    """Pulisce log vecchi / in eccesso (utile quando il disco è quasi pieno)."""
    try:
        out = experimental_assistant._enforce_logs_retention(reason='api')
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
def _gateway_mirror_defaults() -> Dict[str, Any]:
    vehicle = str(os.getenv('KBSM_VEHICLE_MODE', '')).strip().lower() in {'1', 'true', 'yes', 'on'}
    return {
        'enabled': bool(vehicle),
        'autostart': bool(vehicle),
...
def _logs_retention_watchdog_loop() -> None:
    """Retention periodica anche senza incidenti Sentinel (viaggi lunghi)."""
    while True:
        try:
            cfg = config_store.get_config_only() or {}
            ea = cfg.get('experimental_assistant') if isinstance(cfg.get('experimental_assistant'), dict) else {}
            if not bool(ea.get('logs_retention_enabled', cfg.get('logs_retention_enabled', True))):
                time.sleep(300.0)
                continue
            interval_s = float(
                ea.get('logs_retention_min_interval_s')
                or cfg.get('logs_retention_min_interval_s')
                or 300.0
            )
            interval_s = max(60.0, min(interval_s, 3600.0))
            experimental_assistant._enforce_logs_retention(reason='periodic')
            time.sleep(interval_s)
        except Exception as e:
            try:
                print(f'[RetentionWatchdog] {e}', flush=True)
            except Exception:
                pass
            time.sleep(120.0)
...
def _autostart_from_saved_config() -> None:
    ...
    try:
        ...
        # Avvio automatico logging MF4 (viaggio / veicolo lontano dall'operatore)
        auto_log = str(os.getenv('KBSM_AUTO_START_LOGGING', '')).strip().lower() in {'1', 'true', 'yes', 'on'}
        if not auto_log:
            auto_log = str(os.getenv('KBSM_VEHICLE_MODE', '')).strip().lower() in {'1', 'true', 'yes', 'on'}
        if auto_log and not getattr(shared_logger, 'active', False):
            try:
                time.sleep(8.0)
                formats = cfg.get('formats_default')
                if not isinstance(formats, list) or not formats:
                    formats = ['mf4']
                formats = [str(x).strip() for x in formats if str(x).strip()] or ['mf4']
                _ensure_eth_running_for_logging()
                _prepare_gateway_mirror_for_logging()
                manager.start_logging(formats)
                eth_manager.start_logging(formats)
                _log_event('autostart_logging', {'formats': formats})
            except Exception as e:
                try:
                    _log_event('autostart_logging_failed', {'error': str(e)})
                except Exception:
                    pass
    except Exception:
        return
...
if __name__ == '__main__':
    ...
    threading.Thread(target=_logs_retention_watchdog_loop, daemon=True, name='logs-retention').start()
