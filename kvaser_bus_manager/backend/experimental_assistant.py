...
class ExperimentalAssistantService:
    ...
    def __init__(
        ...
        *,
        ...
        # Quando un MIL/spia triggera un incident, il Sentinel del KBM NON vede
        # i frame catturati dal mirror_logger (process separato). Per ottenere
        # l'incident MF4 completo, chiamiamo il suo endpoint
        # /api/incident/snapshot che congela la finestra di mirror.
        # Disabilitabile mettendo MIRROR_LOGGER_INCIDENT_URL=''.
        self._mirror_incident_url = os.getenv(
            'MIRROR_LOGGER_INCIDENT_URL',
            'http://127.0.0.1:5050/api/incident/snapshot',
        ).strip()
        self._mirror_incident_token = os.getenv(
            'MIRROR_LOGGER_TOKEN', '',
        ).strip()
        self._mirror_incident_window_s = max(1, min(int(os.getenv(
            'MIRROR_LOGGER_INCIDENT_WINDOW_S', '45') or '45'), 300))
        self._mirror_incident_timeout_s = max(1.0, float(os.getenv(
            'MIRROR_LOGGER_INCIDENT_TIMEOUT_S', '5') or '5'))
        ...
        self._mil_diag_mode: str = 'passive'   # 'passive' | 'active'
    ...
    def _loop(self) -> None:
        while not self._stop_evt.is_set():
            try:
                ...
                # ── Modalità diagnostica del Sentinel ─────────────────────
                # 'auto' (default): passivo se esiste una mappatura MIL
                #   broadcast, altrimenti polling attivo.
                # 'passive': nessun polling attivo.
                #   Il MIL/spie vengono rilevati dai listener installati su
                #   bus_manager + ethernet_manager: _update_lamps_from_frame
                #   intercetta la transizione OFF→ON del bit MIL nei frame
                #   broadcast e triggera _handle_lamp_incident(kind='mil').
                #   Nessun bus diagnostic occupato, nessun conflitto con
                #   mirror_logger DoIPActivator.
                # 'active': fallback per veicoli che non broadcastano il
                #   MIL ciclicamente. Esegue _poll_mil via CAN o DoIP.
                #   Da usare SOLO se la ECU non emette spontaneamente il
                #   bit MIL e serve interrogarla con OBD-II Mode 01 PID 01
                #   o UDS read DID equivalente.
                diag_mode = str(cfg.get('sentinel_diagnostic_mode', 'auto') or 'auto').strip().lower()
                if diag_mode not in ('auto', 'passive', 'active'):
                    diag_mode = 'auto'
                if diag_mode == 'auto':
                    diag_mode = 'passive' if 'mil' in self._get_lamp_mappings() else 'active'
                self._mil_diag_mode = diag_mode

                if diag_mode == 'passive':
                    # Idle: il trigger arriva via _update_lamps_from_frame
                    # → _handle_lamp_incident(kind='mil'). Niente da fare.
                    time.sleep(0.5)
                    continue
    ...
    def _handle_mil_incident(self, channel_id: int, mil_on_ts_ms: int) -> None:
        ...
        # Snapshot del mirror_logger (cross-process bridge). Best-effort:
        # se il mirror_logger non è attivo o irraggiungibile, il manifest
        # resta vuoto e l'incident del Sentinel completa comunque con i soli
        # dati canlib.
        try:
            mirror_snapshot_manifest = self._request_mirror_snapshot(
                label=f'mil_on_{int(mil_on_ts_ms)}',
            )
        except Exception as e:
            print(f'[Sentinel] _request_mirror_snapshot err: {e}', flush=True)
            mirror_snapshot_manifest = {}
        ...
        incident = {
            ...
            'mirror_snapshot': mirror_snapshot_manifest or {},
    ...
    def _handle_lamp_incident(self, *, kind: str, lamp_on_ts_ms: int, channel: Optional[int]) -> None:
        ...
        # Snapshot mirror_logger (cross-process bridge) — vedi commento in
        # _handle_mil_incident.
        try:
            mirror_snapshot_manifest = self._request_mirror_snapshot(
                label=f"{str(kind or 'lamp')}_on_{int(lamp_on_ts_ms)}",
            )
        except Exception as e:
            print(f'[Sentinel] _request_mirror_snapshot err: {e}', flush=True)
            mirror_snapshot_manifest = {}
    ...
    def _request_mirror_snapshot(self, *, label: str, window_s: Optional[int] = None) -> Dict[str, Any]:
        """Best-effort: chiede al mirror_logger di congelare la finestra di
        mirror corrispondente all'incident.

        Ritorna il `manifest` (dict) se il mirror_logger ha sessione attiva e
        ha prodotto lo snapshot. Ritorna {} se:
          - URL endpoint disabilitato (env MIRROR_LOGGER_INCIDENT_URL='')
          - mirror_logger irraggiungibile / timeout
          - mirror_logger risponde 409 (nessuna sessione attiva)
          - errori vari di rete / JSON

        Non solleva eccezioni: l'incident del Sentinel deve sempre completarsi
        anche se il mirror_logger è giù.
        """
        url = self._mirror_incident_url
        if not url:
            return {}
        ...
        window = int(window_s) if window_s is not None else int(self._mirror_incident_window_s)
        window = max(1, min(window, 300))
        body = _json.dumps({
            'label':    str(label or 'incident'),
            'window_s': window,
        }).encode('utf-8')
        ...
        if self._mirror_incident_token:
            req.add_header('X-Auth-Token', self._mirror_incident_token)

        try:
            with _urlreq.urlopen(req, timeout=self._mirror_incident_timeout_s) as resp:
                payload = _json.loads(resp.read().decode('utf-8'))
        except _urlerr.HTTPError as e:
            if e.code == 409:
                # Nessuna sessione mirror attiva — caso normale, no log
                return {}
            print(f'[Sentinel] mirror snapshot HTTP {e.code}: {e.reason}', flush=True)
            return {}
        except (_urlerr.URLError, OSError, ValueError, TimeoutError) as e:
            print(f'[Sentinel] mirror snapshot non disponibile: {e}', flush=True)
            return {}
        ...
        if not isinstance(manifest, dict):
            return {}
        # Aggiungiamo il path della directory per i consumer downstream
        if 'incident_dir' in payload:
            manifest['incident_dir'] = payload.get('incident_dir')
        return manifest
