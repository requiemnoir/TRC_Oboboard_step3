...
def _copilot_is_rule_wizard_question(user_msg: str) -> bool:
    ...
    if any(k in m for k in ['crea una regola', 'creami una regola', 'aggiungi una regola', 'imposta una regola', 'crea regola', 'create a rule']):
        return True
    # Also accept “regola che attivi una violation se ...”
    if 'regola' in m and any(k in m for k in ['violation', 'violazioni', 'scatta', 'attivi', 'attiva', 'se']):
        return True
    return False
...
DEFAULT_LOG_FOLDER = os.path.join(PROJECT_DIR, 'logs')
...
def _ensure_eth_running_for_logging() -> None:
    ...
    try:
        eth_manager.start({
            ...
            'xcp_ip': str(es.get('target_ip') or '').strip(),
...
def _recording_sync_loop():
    ...
    while True:
        try:
            ...
            # Start video recording once per session (do NOT restart on base_name changes).
            if active and (not last_active or (session_base and session_base != last_session_base)):
                if session_base:
                    ...
                    try:
                        ...
                        pass
                    _log_event('video_recording_started' if started else 'video_recording_failed', {
                        'output': out_path,
...
def _trigger_autostop_loop():
    ...
    while True:
        try:
            ...
            # Backward-compat: some UIs only set a timeout (auto_stop_s) without
            # sending an explicit enable flag.
            implied_enabled = auto_stop_s > 0
            if not (env_enabled or cfg_enabled or implied_enabled):
                time.sleep(0.5)
                continue
    ...
    try:
        ...
        for bus_num in can_buses:
            try:
                ...
                continue
            # The UI uses 1-based CAN labels (CAN1, CAN2, …) but the
            # data-source channel_id is 0-based (0, 1, 2, …).
            # The ECU mirror BusChannel field is also 0-based, so the
            # ethernet_capture emits channel_id = 100 + BusChannel.
            # Map: CAN1 in UI → physical channel 0, virtual mirror 100.
            phys_0 = phys - 1  # convert 1-based UI → 0-based channel_id
            if phys_0 < 0:
                phys_0 = 0
            ...
            if 99 not in new_map:
                new_map[99] = phys_0
            # VAG mirror channel 100+BusChannel → physical channel_id
            new_map[100 + phys_0] = phys_0

        # FlexRay: 'A'→0, 'B'→1, or numeric
...
@app.route('/api/arxmls', methods=['GET'])
def get_arxmls():
    """List ARXML files available in the ARXML database directory."""
    if not os.path.exists(UPLOAD_FOLDER_ARXML):
        return jsonify([])
    out = []
    try:
        for f in os.listdir(UPLOAD_FOLDER_ARXML):
            p = os.path.join(UPLOAD_FOLDER_ARXML, f)
            if not os.path.isfile(p):
                continue
            if not str(f).lower().endswith('.arxml'):
                continue
            out.append(f)
    except Exception:
        return jsonify([])
        ...
        try:
            ...
            for f in os.listdir(folder):
                ...
                if not f.lower().endswith('.mf4'):
                    continue
                # Hide internal chunk artifacts (parts/temp) from user-facing list.
                low = f.lower()
                if low.endswith('.tmp.mf4') or '.tmp.' in low:
                    continue
                # Hide chunk parts only when a corresponding merged base file exists.
                # Otherwise, show them (users may upload already-chunked MF4s).
                m_part = part_re.match(f)
                if m_part:
                    base_name = f"{m_part.group('base')}.mf4"
                    try:
                        if _find_log_file(base_name):
                            continue
                    except Exception:
                        pass
                # Hide Ethernet MF4 artifacts from the default MF4 viewer list.
                # These files are not compatible with the raw CAN decode/plot endpoints.
...
def _resolve_dbc_path(dbc_name: str):
    ...
    if not safe or safe != str(dbc_name or '').strip():
        return None
    path = os.path.join(UPLOAD_FOLDER_DBC, safe)
    return path if os.path.isfile(path) else None


def _mf4_load_raw_table(path: str):
    """Load MF4 raw CAN table.

    Returns `(t_seconds, can_id_u32, dlc_u16, data_u8[N,8], channel_u16, flags_u32)` or `None`.
    """
    import asammdf
    import numpy as np
    ...
    try:
        ...
        dlc_sig = _get_first(['DLC', 'Length', 'DataLength', 'CAN_DataFrame.DLC'])
        ...
        if data_bytes_sig is not None:
            try:
                ...
                if raw.ndim == 2 and raw.shape[1] >= 8:
                    data = raw[:, :8].astype(np.uint8, copy=False)
...
def mf4_list_decoded_signals():
    ...
    use_arxml = str(request.args.get('arxml') or '').strip().lower() in {'1', 'true', 'yes', 'on'}
    fibex_name = str(request.args.get('fibex') or '').strip()
    ...
    if use_arxml:
        ...
        if not dec or not dec.loaded:
            return jsonify({'ok': False, 'error': 'ARXML decoder not loaded (no ARXML file in databases/arxml/)'}), 400
        ...
        try:
            ...
            if not raw:
                return jsonify({'ok': False, 'error': 'mf4 non contiene tabella CAN raw'}), 400
            ...
            if ch_filter_val is not None:
                try:
                    ...
                    pass
            present_ids = set(int(x) for x in np.unique(can_id).tolist())
        except Exception as e:
            return jsonify({'ok': False, 'error': str(e)}), 500

        groups = dec.list_can_signals(only_ids=present_ids)
        # Also include FlexRay signals from ARXML (all available)
        fr_groups = dec.list_fr_signals()
        return jsonify({'ok': True, 'groups': groups + fr_groups, 'source': 'arxml'})

    # ── FIBEX path ──────────────────────────────────────────────────────
    if fibex_name:
        from fibex_loader import FibexLoader
        base = _safe_basename(fibex_name)
        if not base:
            return jsonify({'ok': False, 'error': 'invalid fibex name'}), 400
        allowed_exts = ('.xml', '.fibex', '.arxml')
        if not base.lower().endswith(allowed_exts):
            return jsonify({'ok': False, 'error': 'unsupported file extension'}), 400
        fibex_path = os.path.join(UPLOAD_FOLDER_FIBEX, base)
        if not os.path.isfile(fibex_path):
            return jsonify({'ok': False, 'error': f'fibex not found: {base}'}), 404

        loader = FibexLoader()
        if not loader.load(fibex_path):
            return jsonify({'ok': False, 'error': f'failed to load fibex: {base}'}), 500

        # Build groups from FibexLoader frames/signals
        groups = []
        for slot_id in sorted(loader.frames.keys()):
            msg_name = str(loader.frames.get(slot_id) or f'FR_Slot_{slot_id}')
            defs = loader._signal_defs.get(slot_id) or []
            sig_list = loader.signals.get(slot_id) or []
            sigs = []
            seen = set()
            for d in defs:
                sn = str(d.get('name') or '').strip()
                if not sn or sn in seen:
                    continue
                seen.add(sn)
                sigs.append({'key': f'{msg_name}.{sn}', 'unit': ''})
            if not sigs and sig_list:
                for s in sig_list:
                    ...
                    sigs.append({'key': f'{msg_name}.{sn}', 'unit': ''})
            if sigs:
                groups.append({'message': msg_name, 'signals': sigs})
        return jsonify({'ok': True, 'groups': groups, 'source': 'fibex'})

    # ── DBC path (original) ────────────────────────────────────────────
    if not dbc_names and auto:
        cfg_map = _mf4_configured_dbcs_by_channel()
        if ch_filter_val is not None:
            dbc_names = list(cfg_map.get(int(ch_filter_val), []) or [])
        else:
            # Union across configured channels
            seen = set()
            for lst in (cfg_map.values() if isinstance(cfg_map, dict) else []):
                for n in (lst or []):
    ...
    try:
        ...
        # Merge messages/signals across multiple DBCs
        ...
        for dbc_path in dbc_paths:
            ...
            for m in getattr(db, 'messages', []) or []:
                ...
                if fid not in present_ids:
                    continue
                try:
                    msg_name = str(getattr(m, 'name') or '').strip()
                except Exception:
                    msg_name = ''
                if not msg_name:
                    continue
                mm = merged.get(msg_name)
                if mm is None:
                    mm = {}
                    merged[msg_name] = mm
        ...
        groups = []
        for msg_name in sorted(merged.keys()):
            sig_map = merged[msg_name]
            sigs = [{'key': f'{msg_name}.{sn}', 'unit': str(sig_map.get(sn) or '')} for sn in sorted(sig_map.keys())]
            if sigs:
                groups.append({'message': msg_name, 'signals': sigs})
        return jsonify({'ok': True, 'groups': groups, 'source': 'dbc'})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500


def _despike_series(t_list: list, y_list: list) -> tuple:
    """Remove isolated spike artefacts from a decoded CAN signal.

    When multiple CAN buses are logged simultaneously, frames from different
    ECUs / bus segments can share the same CAN ID.  Decoding such "alien"
    frames with the wrong DBC message produces short-lived outlier values
    ("spikes") that do not represent any real physical quantity.

    Detection — Hampel identifier (local median / MAD):
      For every sample *y[i]* a local median and MAD (Median Absolute
      Deviation) are computed over a symmetric window of ±W neighbours.
      The sample is flagged as a spike when:

          |y[i] – local_median|  >  max(k · 1.4826 · MAD,  ε)

      k     = 5.0   (≈ five-sigma; very conservative to avoid false positives)
      W     = 3     (window half-width → 7 samples total)
      1.4826        consistency constant for Gaussian distributions
      ε     = adaptive floor = max(1.0, 2% of the signal's global range)

    Boundary samples (first two / last two) are always preserved.
    Returns (t_filtered, y_filtered) — new lists with spikes removed.
    """
    n = len(y_list)
    if n < 7:
    ...
    for v in y_list:
        ...
        if v > y_max:
            y_max = v
    eps = max(1.0, (y_max - y_min) * 0.02)

    keep = [True] * n
    for i in range(W, n - W):
        lo = i - W
        hi = i + W + 1
        window = sorted(y_list[lo:hi])
        wn = len(window)
        med = window[wn >> 1]
        deviations = sorted(abs(v - med) for v in window)
        mad = deviations[wn >> 1]
        threshold = max(K * C * mad, eps)
        if abs(y_list[i] - med) > threshold:
            keep[i] = False

    t_out = [t_list[i] for i in range(n) if keep[i]]
    y_out = [y_list[i] for i in range(n) if keep[i]]
    return t_out, y_out
...
def mf4_get_decoded_data():
    ...
    use_arxml = bool(data_in.get('arxml'))
    fibex_name = str(data_in.get('fibex') or '').strip()
    ...
    if (not dbc_name) and (not dbc_names) and (not auto) and (not use_arxml) and (not fibex_name):
        return jsonify({'ok': False, 'error': 'missing dbc/arxml/fibex'}), 400
    if not isinstance(signals, list) or not signals:
        return jsonify({'ok': False, 'error': 'signals must be a non-empty list'}), 400
    ...
    # Spike filter: enabled by default; disable with spike_filter=false
    spike_filter = str(data_in.get('spike_filter', 'true')).strip().lower() not in ('false', '0', 'no', 'off')

    mf4_path = _find_log_file(filename)
    if not mf4_path:
        return jsonify({'ok': False, 'error': 'file not found'}), 404

    # ── ARXML / FIBEX decode paths ──────────────────────────────────────
    if use_arxml or fibex_name:
        import numpy as np

        # Parse requested keys into message -> signal set
        req = {}
        for k in signals:
            if '.' not in k:
                continue
        ...
        try:
            ...
            if ch_filter_val is not None:
                try:
                    mask = (ch == int(ch_filter_val))
                    t_abs = t_abs[mask]; can_id = can_id[mask]; dlc = dlc[mask]
                    payload = payload[mask]; ch = ch[mask]; fl = fl[mask]
                except Exception:
                    pass
            ...
            t_abs = t_abs[i0:i1]; can_id = can_id[i0:i1]; dlc = dlc[i0:i1]; payload = payload[i0:i1]
            if t_abs.size == 0:
                return jsonify({'ok': True, 'series': []})

            out = {k: {'name': k, 'unit': '', 't': [], 'y': []} for k in signals if '.' in k}

            # ── Pre-filter to only CAN IDs that carry requested messages ──
            # This is critical: without it the downsample below would
            # blindly stride across ALL CAN traffic and keep very few
            # frames for sparsely-occurring message IDs, producing
            # multi-second gaps between data points in the viewer.
            if use_arxml:
                dec = getattr(manager, 'arxml_decoder', None)
                if not dec or not dec.loaded:
                    return jsonify({'ok': False, 'error': 'ARXML decoder not loaded'}), 400

                from arxml_decoder import _clean_frame_name
                wanted_ids = set()
                for fid, entries in dec._can_index.items():
                    for entry in entries:
                        mn = _clean_frame_name(entry.frame_name)
                        if mn in req:
                            wanted_ids.add(int(fid))
                            break

                if wanted_ids:
                    wanted_arr = np.fromiter((int(x) for x in wanted_ids), dtype=can_id.dtype)
                    mask = np.isin(can_id, wanted_arr)
                    t_abs = t_abs[mask]; can_id = can_id[mask]; dlc = dlc[mask]; payload = payload[mask]
                    if t_abs.size == 0:
                        return jsonify({'ok': True, 'series': []})

            elif fibex_name:
                # For FIBEX we cannot easily pre-filter IDs without
                # loading the decoder first, but we do it below when
                # available. The stratified downsample still helps.
                pass

            # ── Stratified downsample per CAN-ID ──────────────────────
            # Same algorithm used by the DBC path: distribute the
            # max_points budget proportionally across CAN IDs so that
            # low-frequency messages are not starved by high-frequency ones.
            if int(t_abs.size) > max_points:
                try:
                    total = int(t_abs.size)
                    ids_u, counts = np.unique(can_id, return_counts=True)
                    if ids_u.size > 0:
                        quotas = np.maximum(
                            1,
                        ...
                        chosen = []
                        for fid_u, q in zip(ids_u.tolist(), quotas.tolist()):
                            idxs = np.nonzero(can_id == fid_u)[0]
                            nn = int(idxs.size)
                            if nn <= 0:
                                continue
                            if q >= nn:
                                chosen.append(idxs)
                                continue
                            take = np.linspace(0, nn - 1, num=int(q), dtype=int)
                            chosen.append(idxs[take])

                        if chosen:
                            keep = np.unique(np.concatenate(chosen))
                            keep.sort()
                            t_abs = t_abs[keep]; can_id = can_id[keep]; dlc = dlc[keep]; payload = payload[keep]
                except Exception:
                    step = max(1, int(np.ceil(float(t_abs.size) / float(max_points))))
                    t_abs = t_abs[::step]; can_id = can_id[::step]; dlc = dlc[::step]; payload = payload[::step]

            t0 = float(t_abs[0])
            t_out = t_abs.astype(np.float64, copy=False) if t_mode in {'abs', 'absolute'} else (t_abs - t0).astype(np.float64, copy=False)
            ...
            if use_arxml:
                ...
                # Build unit map from decoder index
                from arxml_decoder import _clean_frame_name
                for fid, entries in dec._can_index.items():
                    for entry in entries:
                        mn = _clean_frame_name(entry.frame_name)
                        if mn not in req:
                            continue
                        for sd in entry.signals:
                            key = f'{mn}.{sd.name}'
                            if key in out:
                                out[key]['unit'] = sd.unit or ''

                for idx in range(int(t_abs.size)):
                    fid = int(can_id[idx])
                    ln = min(8, max(int(dlc[idx]), 0))
                    b = bytes(payload[idx][:ln].tolist())
                    decoded = dec.decode(fid, b)
                    ...
                    for sn in needed:
                        key = f'{msg_name}.{sn}'
                        val = decoded['signals'].get(sn)
                        if val is None:
                            continue
                        try:
                            fv = float(val) if not isinstance(val, str) else None
                        ...
                        try:
                            ...
                            continue
                        if key not in out:
                            out[key] = {'name': key, 'unit': '', 't': [], 'y': []}
                        out[key]['t'].append(float(t_out[idx]))
                        out[key]['y'].append(fv)

            elif fibex_name:
                from fibex_loader import FibexLoader
                fbase = _safe_basename(fibex_name)
                if not fbase:
                    return jsonify({'ok': False, 'error': 'invalid fibex name'}), 400
                fibex_path = os.path.join(UPLOAD_FOLDER_FIBEX, fbase)
                if not os.path.isfile(fibex_path):
                    return jsonify({'ok': False, 'error': f'fibex not found: {fbase}'}), 404
                loader = FibexLoader()
                if not loader.load(fibex_path):
                    return jsonify({'ok': False, 'error': f'failed to load fibex: {fbase}'}), 500

                for idx in range(int(t_abs.size)):
                    fid = int(can_id[idx])
                    ln = min(8, max(int(dlc[idx]), 0))
                    b = bytes(payload[idx][:ln].tolist())
                    decoded = loader.decode(fid, b)
                    if not decoded or not decoded.get('signals'):
                        continue
                    msg_name = decoded.get('name', '')
                    needed = req.get(msg_name)
            ...
            if spike_filter:
                for s in series:
                    s['t'], s['y'] = _despike_series(s['t'], s['y'])
                series = [s for s in series if s['t'] and s['y']]
            return jsonify({'ok': True, 'series': series})
        except Exception as e:
            return jsonify({'ok': False, 'error': str(e)}), 500

    # ── DBC decode path (original) ─────────────────────────────────────
    # Resolve DBC list.
    ...
    if not dbc_names and auto:
        ...
        if ch_filter_val is not None:
            ...
            # Union across configured channels (used for units/messages lookup)
            seen = set()
            dbc_names = []
            for lst in (cfg_map.values() if isinstance(cfg_map, dict) else []):
                for n in (lst or []):
    ...
    # Parse requested keys into message -> signal list
    req = {}
    for k in signals:
        if '.' not in k:
            continue
    ...
    try:
        ...
        # Filter to only frames we can decode for the requested messages.
        wanted_ids = set()
        for msg_name in req.keys():
            m = messages_by_name.get(msg_name)
            if not m:
                continue
            try:
                wanted_ids.add(int(getattr(m, 'frame_id')))
            except Exception:
                continue
        ...
        units = {}
        for msg_name, sigs in req.items():
            m = messages_by_name.get(msg_name)
            if not m:
                continue
            for s in getattr(m, 'signals', []) or []:
                try:
                    sn = str(getattr(s, 'name') or '').strip()
                    if sn in sigs:
                        units[f'{msg_name}.{sn}'] = str(getattr(s, 'unit') or '').strip()
                except Exception:
                    continue

        out = {k: {'name': k, 'unit': units.get(k, ''), 't': [], 'y': []} for k in signals if '.' in k}
        ...
        for idx in range(int(t_rel.size)):
            ...
            try:
                ...
                dl = ml
            # Skip frames whose DLC is shorter than the expected message
            # length.  DLC=0 ghost/RTR frames carry no payload; decoding
            # them produces all-zero signals (false 0 km/h spikes, etc.).
            if dl < ml:
                continue
            ln = max(0, min(8, max(ml, min(dl, 8))))
            b = bytes(payload[idx][:ln].tolist())

            try:
                try:
                    decoded = m.decode(b, decode_choices=False)
                except TypeError:
                    decoded = m.decode(b)
            except Exception:
                continue
            ...
            for sn in needed:
                key = f'{msg_name}.{sn}'
                if key not in out:
                    out[key] = {'name': key, 'unit': units.get(key, ''), 't': [], 'y': []}
                try:
                    val = decoded.get(sn)
                except Exception:
                    val = None
...
def mf4_export_decoded_mf4():
    """Export a new MF4 containing decoded signals computed from a raw MF4 using DBC(s).

    Body:
      - file: source mf4 in logs
      - dbcs: optional list of DBC names (basename)
      - auto: bool; when true and dbcs empty uses config mapping
      - channel: optional int; if set exports only that CAN Channel
      - signals: list of keys "Message.Signal" to export (required)
      - start_s/end_s: optional window in seconds relative to first raw sample
    """
    ...
    if isinstance(dbc_names, list):
        ...
        dbc_names = []
    auto = bool(data_in.get('auto'))
    ch_filter = data_in.get('channel', None)
    ch_filter_val = None
    if ch_filter is not None and str(ch_filter).strip() != '':
        try:
            ch_filter_val = int(str(ch_filter).strip())
    ...
    if not filename or not filename.lower().endswith('.mf4'):
        return jsonify({'ok': False, 'error': 'invalid file'}), 400
    if (not dbc_names) and (not auto):
        return jsonify({'ok': False, 'error': 'missing dbc'}), 400
    if not isinstance(signals, list) or not signals:
        return jsonify({'ok': False, 'error': 'signals must be a non-empty list'}), 400
    signals = [str(s).strip() for s in signals if str(s).strip() and '.' in str(s)]
    if not signals:
        return jsonify({'ok': False, 'error': 'no decoded signals selected'}), 400
    ...
    for k in signals:
        ...
        if msg and sig:
            req.setdefault(msg, set()).add(sig)
    if not req:
        return jsonify({'ok': False, 'error': 'no valid decoded signal keys'}), 400

    try:
        raw = _mf4_load_raw_table(mf4_path)
        if not raw:
            return jsonify({'ok': False, 'error': 'mf4 non contiene tabella CAN raw (CAN_ID/DLC/DataByte*). Questo file sembra già “misurato/decodificato” (stile ETAS/INCA).'}), 400
        t_abs, can_id, dlc, payload, ch_arr, fl_arr = raw

        if ch_filter_val is not None:
            try:
                mask = (ch_arr == int(ch_filter_val))
                t_abs = t_abs[mask]
                can_id = can_id[mask]
                dlc = dlc[mask]
                payload = payload[mask]
        ...
        if t_abs.size == 0:
            return jsonify({'ok': False, 'error': 'no data in selected time/channel window'}), 400
        ...
        ch_arr = ch_arr[i0:i1]
        if t_abs.size == 0:
            return jsonify({'ok': False, 'error': 'no data in selected time window'}), 400
        ...
        units: dict[str, str] = {}
        for msg_name, sigs in req.items():
            m0 = messages_by_name.get(msg_name)
            if not m0:
                continue
            for s in getattr(m0, 'signals', []) or []:
                try:
                    sn = str(getattr(s, 'name') or '').strip()
                    if sn in sigs:
                        units[f'{msg_name}.{sn}'] = str(getattr(s, 'unit') or '').strip()
        ...
        # Filter frames to only requested message IDs (best-effort)
        wanted_ids = set()
        for msg_name in req.keys():
            m0 = messages_by_name.get(msg_name)
            if not m0:
                continue
            try:
                wanted_ids.add(int(getattr(m0, 'frame_id')))
            except Exception:
                continue
        if wanted_ids:
            try:
                ...
                pass
        if t_rel.size == 0:
            return jsonify({'ok': False, 'error': 'no decodable frames for selected signals'}), 400

        # Buffers keyed by (channel, key)
        buffers: dict[tuple[int, str], dict[str, object]] = {}
        ...
        for idx in range(int(t_rel.size)):
            ...
            try:
                ml = int(getattr(msg, 'length', 8) or 8)
            except Exception:
                ml = 8
            try:
                dl = int(dlc[idx])
            ...
            try:
                try:
                    decoded = msg.decode(b, decode_choices=False)
                except TypeError:
                    decoded = msg.decode(b)
            except Exception:
                continue

            ch_id = 0
            try:
                ch_id = int(ch_arr[idx])
            except Exception:
                ch_id = 0
        ...
        if not buffers:
            return jsonify({'ok': False, 'error': 'no numeric decoded samples for selected signals'}), 400
        ...
        out_name = f'{stem}_decoded_{ts}.mf4'
        # IMPORTANT: asammdf may auto-append ".mf4" if the path doesn't end with ".mf4".
        # So keep the temporary filename ending with ".mf4" to ensure atomic rename works.
        out_tmp_name = f'{stem}_decoded_{ts}.tmp.mf4'
        out_path_tmp = os.path.join(out_dir, out_tmp_name)
        out_path = os.path.join(out_dir, out_name)

        mdf_out = asammdf.MDF(version='4.10')
        try:
            for (ch_id, key), buf in sorted(buffers.items(), key=lambda it: (it[0][0], it[0][1])):
                ...
                acq = _safe_mf4_group_name(f'{key}_R{ch_id}')
                time_sig = Signal(samples=t_vec, timestamps=t_vec, name='time', unit='s')
                val_sig = Signal(samples=y_vec, timestamps=t_vec, name=key, unit=unit)
                mdf_out.append([time_sig, val_sig], acq_name=acq)

            try:
                mdf_out.save(out_path_tmp, overwrite=True)

                # Robustly locate the saved file (asammdf can auto-append extension).
                saved_path = out_path_tmp
                if not os.path.exists(saved_path):
                    alt = out_path_tmp + '.mf4'
    ...
    def _iter_csv():
        ...
        for i in range(int(n)):
            ...
            for m_by_id in db_maps:
                ...
                if not m:
                    continue
                # Skip frames whose DLC is shorter than the DBC-defined
                # message length — they carry no valid payload (RTR/ghost).
                try:
                    ml = int(getattr(m, 'length', 8) or 8)
                except Exception:
                    ml = 8
...
def scantools_run():
    ...
    # DoIP actions are Ethernet-based and do not require CAN channel to be active.
    if action in ('vag_doip_scan_report', 'doip_clear_dtcs', 'doip_mode06'):
        cfg = {}
        try:
            cfg = config_store.get_config_only() or {}
        except Exception:
    ...
    # NOTE: doip_clear_dtcs and doip_mode06 are handled above (same DoIP block).
...
def delete_log(filename):
    ...
    if safe_name in {'webapp.out', 'webapp.pid'}:
...
def inject_can_frame():
    ...
    try:
        ...
        frame_type = str(data.get('frame_type', 'CAN')).strip().upper() or 'CAN'
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 400

    try:
        manager.inject_frame(channel_id, arb_id, payload, flags=0, frame_type=frame_type)
        return jsonify({'ok': True})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
def start_log():
    ...
    try:
        ...
        pass
    data = request.json or {}
    formats = data.get('formats', ['csv', 'txt'])
    _ensure_eth_running_for_logging()
    manager.start_logging(formats)
    eth_manager.start_logging(formats)
    _log_started_by_trigger = False
...
def arxml_reload_catalog():
    ...
    try:
        from arxml_parser import load_catalog_from_directory
        cat = load_catalog_from_directory(UPLOAD_FOLDER_ARXML)
        # Also reload the bus manager's ARXML decoder
        try:
            n = manager.load_arxml_catalog(cat)
        except Exception:
            n = 0
        summary = cat.summary()
        summary['arxml_decoder_frames'] = n
        return jsonify({'ok': True, 'catalog': summary})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)}), 500
...
def gateway_mirror_stop():
    # [TRC Fix 2026-02-16] Safety guard removed — SFD authentication
    # (RoutineControl 0x0253) enables reliable restarts without KL15
    # power cycle.  The mirror can be stopped and restarted from the UI.
    data = request.get_json(silent=True) or {}
    data = data if isinstance(data, dict) else {}
    ...
    if out.get('ok'):
        out['info'] = 'Mirror stopped. Use Start Mirror to re-enable.'
    return jsonify(out), code
