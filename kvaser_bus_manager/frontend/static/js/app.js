...
const acqInputsStatusEl = document.getElementById('acq-inputs-status');
const acqConfigStatusEl = document.getElementById('acq-config-status');
const liveTrafficSourceHintEl = document.getElementById('live-traffic-source-hint');
...
async function refreshLoggingStatus() {
    try {
        ...
        // Update KL_15 status from log/status response
        _updateKl15StatusFromLogStatus(data);
        _updateAcquisitionInputStatus(data);
    } catch (e) {
        // ignore
    }
}

function _formatChannelList(channels) {
    if (!Array.isArray(channels) || !channels.length) return 'none';
    return channels.map(ch => `CAN${ch}`).join(', ');
}

function _formatMirrorBusList(buses) {
    if (!Array.isArray(buses) || !buses.length) return 'none';
    return buses.map(bus => `net ${bus}`).join(', ');
}

function _updateAcquisitionInputStatus(data) {
    try {
        const inputs = data?.inputs || {};
        const busRunning = !!inputs.bus_running;
        const busChannels = Array.isArray(inputs.bus_channels) ? inputs.bus_channels : [];
        const ethRunning = !!inputs.eth_running;
        const ethInterface = String(inputs.eth_interface || '').trim() || 'eth';
        const mirrorEnabled = !!inputs.gateway_mirror_enabled;
        const mirrorCan = Array.isArray(inputs.gateway_mirror_can) ? inputs.gateway_mirror_can : [];
        const loggerChannels = Array.isArray(inputs.logger_channels_config) ? inputs.logger_channels_config : [];

        const activeParts = [];
        if (busRunning && busChannels.length) {
            activeParts.push(`Local CAN active: ${_formatChannelList(busChannels)}`);
        } else {
            activeParts.push('Local CAN inactive');
        }
        if (mirrorEnabled && ethRunning) {
            activeParts.push(`Mirror active on ${ethInterface}: ${_formatMirrorBusList(mirrorCan)}`);
        } else if (ethRunning) {
            activeParts.push(`Ethernet capture active on ${ethInterface}`);
        } else {
            activeParts.push('Ethernet inactive');
        }

        if (acqInputsStatusEl) {
            acqInputsStatusEl.textContent = `Active inputs: ${activeParts.join(' | ')}`;
        }
        if (acqConfigStatusEl) {
            acqConfigStatusEl.textContent = `UI logger channels: ${loggerChannels.length ? _formatChannelList(loggerChannels) : 'none'} | Logical source mapping remains separate`;
        }
        if (liveTrafficSourceHintEl) {
            liveTrafficSourceHintEl.textContent = mirrorEnabled
                ? 'Live rows keep virtual mirror channel IDs and add a mirror origin tag'
                : 'Live rows show the active local input channels';
        }
    } catch (_) {
        // ignore
    }
...
function _updateKl15StatusFromLogStatus(data) {
    try {
        const kl = data?.kl15;
        if (!kl) return;
        const badge = document.getElementById('kl15-status-badge');
        const icon = document.getElementById('kl15-icon');
        if (badge) {
            if (!kl.enabled) {
                badge.textContent = 'Disabled';
                badge.className = 'badge text-bg-secondary';
            } else if (kl.recording) {
                badge.textContent = 'Recording (KL_15 ON)';
                badge.className = 'badge text-bg-danger';
            } else if (kl.detected) {
                badge.textContent = 'Ignition ON';
                badge.className = 'badge text-bg-success';
            } else {
                badge.textContent = 'Waiting for ignition…';
                badge.className = 'badge text-bg-warning';
            }
        }
        if (icon) {
            icon.textContent = kl.detected ? '🟢' : '🔑';
        }
        const valEl = document.getElementById('kl15-last-value');
        if (valEl) valEl.textContent = (kl.last_value != null) ? String(kl.last_value) : '—';
    } catch (_) {
        // ignore
    }
}
...
function initKl15Monitor() {
    const enabledEl = document.getElementById('kl15-enabled');
    const applyBtn = document.getElementById('btn-kl15-apply');
    const detailsEl = document.getElementById('kl15-details');
    if (!enabledEl) return;
    ...
    if (applyBtn) {
        applyBtn.addEventListener('click', async () => {
            const msgFilter = document.getElementById('kl15-msg-filter')?.value || '';
            const threshold = parseFloat(document.getElementById('kl15-threshold')?.value) || 0.5;
            const debounce = parseFloat(document.getElementById('kl15-debounce')?.value) || 3.0;
            const signalsRaw = document.getElementById('kl15-signals')?.value || '';
            const signalNames = signalsRaw.split(',').map(s => s.trim()).filter(Boolean);
            const formats = getSelectedFormats();
            try {
                await fetch('/api/kl15', {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        ...
                        message_filter: msgFilter,
                        on_threshold: threshold,
                        off_debounce_s: debounce,
                        signal_names: signalNames.length ? signalNames : undefined,
                        formats: formats,
                    })
                });
            } catch (_) {}
    ...
    (async () => {
        try {
            const res = await fetch('/api/kl15', { cache: 'no-store' });
            if (!res.ok) return;
            const data = await res.json();
            const cfg = data?.config || {};
            enabledEl.checked = !!cfg.enabled;
            if (detailsEl) detailsEl.style.display = cfg.enabled ? '' : 'none';
            const msgEl = document.getElementById('kl15-msg-filter');
            if (msgEl) msgEl.value = cfg.message_filter || '';
            const threshEl = document.getElementById('kl15-threshold');
            if (threshEl) threshEl.value = cfg.on_threshold ?? 0.5;
            const debEl = document.getElementById('kl15-debounce');
            if (debEl) debEl.value = cfg.off_debounce_s ?? 3.0;
            const sigEl = document.getElementById('kl15-signals');
            if (sigEl) sigEl.value = (cfg.signal_names || []).join(',');

            const state = data?.state || {};
            const sigNameEl = document.getElementById('kl15-last-signal');
            const sigValEl = document.getElementById('kl15-last-value');
            const sigMsgEl = document.getElementById('kl15-last-msg');
            if (sigNameEl) sigNameEl.textContent = state.last_signal_name || '—';
            if (sigValEl) sigValEl.textContent = (state.last_value != null) ? String(state.last_value) : '—';
            if (sigMsgEl) sigMsgEl.textContent = state.last_message_name || '—';
        } catch (_) {}
    })();

    // Poll KL_15 details periodically
    setInterval(async () => {
...
async function loadAppConfig() {
    ...
    // Restore MF4 part time limit (minutes)
    try {
        const el = document.getElementById('mf4-part-time-limit-min');
        if (el) {
            const v = (cfg && cfg.mf4_part_time_limit_min !== undefined && cfg.mf4_part_time_limit_min !== null) ? Number(cfg.mf4_part_time_limit_min) : 10;
            if (Number.isFinite(v) && v >= 0) el.value = String(Math.round(v));
            else el.value = '10';
            try { el.classList.remove('is-invalid'); } catch (e) { /* ignore */ }
        }
    ...
    // Restore MF4 flush interval (MB)
    try {
        const el = document.getElementById('mf4-flush-interval-mb');
        if (el) {
            const v = (cfg && cfg.mf4_flush_interval_mb !== undefined && cfg.mf4_flush_interval_mb !== null) ? Number(cfg.mf4_flush_interval_mb) : 10;
            if (Number.isFinite(v) && v > 0) el.value = String(Math.round(v));
            else el.value = '10';
            try { el.classList.remove('is-invalid'); } catch (e) { /* ignore */ }
        }
...
function initConfigPersistence() {
    ...
    // Persist MF4 part time limit (minutes)
    const mf4PartTimeEl = document.getElementById('mf4-part-time-limit-min');
    const persistMf4PartTime = () => {
        if (!mf4PartTimeEl) return;
        const raw = String(mf4PartTimeEl.value || '').trim();
        if (!raw) return;
        const v = Number(raw);
        if (!Number.isFinite(v) || v < 0) {
            try { mf4PartTimeEl.classList.add('is-invalid'); } catch (e) { /* ignore */ }
            return;
        }
        const clamped = Math.max(0, Math.min(1440, Math.round(v)));
        if (clamped !== Math.round(v)) {
            try { mf4PartTimeEl.value = String(clamped); } catch (e) { /* ignore */ }
        }
        try { mf4PartTimeEl.classList.remove('is-invalid'); } catch (e) { /* ignore */ }
        debounceSaveConfig({ mf4_part_time_limit_min: clamped });
    };
    if (mf4PartTimeEl) {
        mf4PartTimeEl.addEventListener('change', persistMf4PartTime);
        mf4PartTimeEl.addEventListener('input', persistMf4PartTime);
    }

    // Persist MF4 intermediate flush interval (MB)
    const mf4FlushEl = document.getElementById('mf4-flush-interval-mb');
    const persistMf4Flush = () => {
        if (!mf4FlushEl) return;
        const raw = String(mf4FlushEl.value || '').trim();
        if (!raw) return;
        const v = Number(raw);
        if (!Number.isFinite(v) || v <= 0) {
            try { mf4FlushEl.classList.add('is-invalid'); } catch (e) { /* ignore */ }
            return;
        }
        const clamped = Math.max(1, Math.min(4096, Math.round(v)));
        if (clamped !== Math.round(v)) {
            try { mf4FlushEl.value = String(clamped); } catch (e) { /* ignore */ }
        }
        try { mf4FlushEl.classList.remove('is-invalid'); } catch (e) { /* ignore */ }
        debounceSaveConfig({ mf4_flush_interval_mb: clamped });
    };
    if (mf4FlushEl) {
        mf4FlushEl.addEventListener('change', persistMf4Flush);
        mf4FlushEl.addEventListener('input', persistMf4Flush);
    }
...
function timelineSetVideoModeReview() {
    ...
    fetch('/api/timeline/live_stream', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function timelineRefreshSignals() {
    ...
    if (mode === 'review') {
        ...
        // Prefer CAN raw->DBC decoded listing when applicable, but fall back to listing
        // signals directly from the MF4 (covers measured MF4 and FlexRay-decoded channels).
        let info = null;
        try {
            info = await mf4FetchJson(`/api/mf4/info?file=${encodeURIComponent(timelineReviewFile)}`);
        } catch (_) {
            info = null;
        }
        const isRawCan = !!(info && info.ok && String(info.kind) === 'raw_can');
        const hasDecodedChannels = !!(info && info.ok && info.has_decoded_channels);
        const useEmbeddedDecoded = hasDecodedChannels && dbcs.length === 0;
        const wantDecodedList = isRawCan && !useEmbeddedDecoded && ((auto && dbcs.length === 0) || (Array.isArray(dbcs) && dbcs.length > 0));
        ...
        if (wantDecodedList) {
            ...
            try {
                ...
                // If the backend says raw table missing / dbc missing, fall through to direct list.
            } catch (e) {
                console.error(e);
        ...
        // Direct MF4 signal listing (includes FlexRay channels if present).
        try {
            const direct = await mf4FetchJson(`/api/mf4/signals?file=${encodeURIComponent(timelineReviewFile)}`);
            if (!direct?.ok || !Array.isArray(direct.signals)) {
                timelineSetStatus(direct?.error ? String(direct.error) : 'Failed to load signals', true);
                return;
            }
            const names = (direct.signals || []).map(s => String(s || '').trim()).filter(Boolean);
            const byGroup = new Map();
            for (const n of names) {
                const idx = n.indexOf('.');
                const g = (idx > 0) ? n.slice(0, idx) : 'Other';
                const lbl = (idx > 0) ? n.slice(idx + 1) : n;
                if (!byGroup.has(g)) byGroup.set(g, []);
                byGroup.get(g).push({ key: n, label: lbl });
            }
            const groups = Array.from(byGroup.keys()).sort((a, b) => String(a).localeCompare(String(b)));
            for (const g of groups) {
                const items = byGroup.get(g) || [];
                if (!items.length) continue;
                // Keep groups stable and readable.
...
async function timelinePlot() {
    ...
    if (mode === 'review') {
        ...
        try {
            ...
            const hasDecodedChannels = !!(info && info.ok && info.has_decoded_channels);
            const useEmbeddedDecoded = hasDecodedChannels && dbcs.length === 0;
            const wantDecoded = isRawCan && !useEmbeddedDecoded && (Array.isArray(dbcs) && dbcs.length > 0 || !!auto);
...
function setMode(mode) {
    ...
    if (showSettings) {
        ...
        try { if (typeof loadTrcConfig === 'function') loadTrcConfig(); } catch (_) {}
    }
}
...
async function gmStop() {
    ...
    gmSetResponse(null);
    try {
        const res = await fetch('/api/gateway/mirror/stop', { method: 'POST' });
        const data = await res.json();
        gmSetResponse(data);
        if (data?.ok) {
            gmSetStatus('Stopped (disabled)');
            // Refresh UI fields in case backend auto-resolved values
            if (data.target_addr && gmTargetAddrEl) gmTargetAddrEl.value = String(data.target_addr);
        } else {
...
let mf4RawExportAvailable = false;
...
function mf4FormatFileSize(bytes) {
...
function mf4BuildLogHref(relativeName) {
    const raw = String(relativeName || '').trim();
    if (!raw) return '#';
    return '/api/logs/' + raw.split('/').filter(Boolean).map(seg => encodeURIComponent(seg)).join('/');
}

function mf4GetFileEntries() {
    return Array.isArray(mf4FilesCache) ? mf4FilesCache : [];
}

function mf4SetFileMenuPlaceholder(selectEl, text) {
    if (!selectEl) return;
    selectEl.innerHTML = '';
    const opt = document.createElement('option');
    opt.value = '';
    opt.disabled = true;
    opt.selected = true;
    opt.text = String(text || 'No files found');
    selectEl.appendChild(opt);
}

function mf4PopulateFileMenu(selectEl, entries, emptyText) {
    if (!selectEl) return;
    const items = Array.isArray(entries) ? entries : [];

    // Also populate the custom expandable picker list if it exists
    const pickerId = selectEl.id.replace('-select', '-list');
    const pickerList = document.getElementById(pickerId);
    const toggleId = selectEl.id.replace('-select', '-toggle');
    const toggleBtn = document.getElementById(toggleId);

    if (!items.length) {
        mf4SetFileMenuPlaceholder(selectEl, emptyText);
        if (pickerList) {
            pickerList.innerHTML = '';
            const li = document.createElement('li');
            li.className = 'empty-placeholder';
            li.textContent = emptyText || 'No files found';
            pickerList.appendChild(li);
        }
        if (toggleBtn) {
            toggleBtn.querySelector('.mf4-file-picker-label').textContent = emptyText || 'No files found';
        }
        return;
    }
    ...
    items.forEach((entry) => {
        ...
        if (!name) return;
        const basename = String(entry?.basename || name.split('/').pop() || name).trim();

        // Hidden select option (keeps downstream code working)
        const opt = document.createElement('option');
        opt.value = name;
        opt.text = basename;
        opt.title = name;
        ...
        if (pickerList) {
            ...
            li.addEventListener('click', () => {
                mf4FilePickerSelect(selectEl, li);
            });
            pickerList.appendChild(li);
        }
    });
    ...
    if (toggleBtn) {
        toggleBtn.querySelector('.mf4-file-picker-label').textContent = 'Select a file…';
    }
}

/** Select a file from the custom picker, sync hidden select, fire change. */
function mf4FilePickerSelect(selectEl, li) {
    const value = li.dataset.value;
    const list = li.closest('.mf4-file-picker-list');
    const picker = li.closest('.mf4-file-picker');
    ...
    // Update toggle label
    const toggle = picker?.querySelector('.mf4-file-picker-toggle');
    if (toggle) toggle.querySelector('.mf4-file-picker-label').textContent = li.textContent;

    // Collapse
    if (picker) picker.classList.remove('open');
    if (toggle) toggle.setAttribute('aria-expanded', 'false');

    // Sync hidden select and fire its change event
    if (selectEl) {
...
function mf4SyncViewerFileMenus(activeFile) {
    const rawSelect = document.getElementById('mf4-raw-files-select');
    const exportedSelect = document.getElementById('mf4-exported-files-select');
    const current = String(activeFile || document.getElementById('mf4-file-select')?.value || '').trim();

    const applySelection = (selectEl, value) => {
        if (!selectEl) return false;
        const exists = Array.from(selectEl.options || []).some((opt) => String(opt.value || '').trim() === value);
        if (!exists) {
            try {
                selectEl.selectedIndex = -1;
            } catch (_) {
    ...
    const rawActive = applySelection(rawSelect, current);
    const exportedActive = rawActive ? false : applySelection(exportedSelect, current);

    if (rawActive && exportedSelect) {
        try {
            exportedSelect.selectedIndex = -1;
        } catch (_) {
            // ignore
        }
    } else if (exportedActive && rawSelect) {
        try {
            rawSelect.selectedIndex = -1;
        } catch (_) {
            // ignore
        }
    }

    // Sync custom expandable pickers to match hidden selects
    mf4SyncPickerUI('mf4-raw-files', rawActive ? current : null);
    mf4SyncPickerUI('mf4-exported-files', exportedActive ? current : null);
}

/** Sync a custom picker's active item and toggle label from a value. */
function mf4SyncPickerUI(prefix, activeValue) {
    const list = document.getElementById(prefix + '-list');
    const toggle = document.getElementById(prefix + '-toggle');
    if (!list) return;
    let found = false;
    list.querySelectorAll('li[data-value]').forEach((li) => {
        if (activeValue && li.dataset.value === activeValue) {
            li.classList.add('active');
            found = true;
            if (toggle) toggle.querySelector('.mf4-file-picker-label').textContent = li.textContent;
        } else {
    ...
    if (!found && toggle) {
        toggle.querySelector('.mf4-file-picker-label').textContent = 'Select a file…';
    }
}

async function mf4ActivateFile(fileName) {
    const fileSelect = document.getElementById('mf4-file-select');
    const target = String(fileName || '').trim();
    if (!fileSelect || !target) return false;
    const exists = Array.from(fileSelect.options || []).some(o => String(o.value || '').trim() === target);
    if (!exists) return false;
    fileSelect.value = target;
    mf4SyncViewerFileMenus(target);
    mf4SaveState({ file: target, signals: [], messages: [] });
    await mf4LoadSignalsForSelectedFile();
    return true;
}

async function mf4HandleViewerFileMenuChange(selectEl) {
    const target = String(selectEl?.value || '').trim();
    if (!target) return;
    const opened = await mf4ActivateFile(target);
    if (!opened) {
        mf4SetStatus('Unable to open selected MF4 file', true);
    }
}
...
function initMf4Viewer() {
    ...
    const fileSelect = document.getElementById('mf4-file-select');
    const rawFilesSelect = document.getElementById('mf4-raw-files-select');
    const exportedFilesSelect = document.getElementById('mf4-exported-files-select');
    ...
    if (dbcSelect) dbcSelect.addEventListener('change', async () => {
        ...
        const { auto, dbcs } = mf4GetSelectedDbcs();
    ...
    if (fileSelect) fileSelect.addEventListener('change', async () => {
        ...
        mf4SyncViewerFileMenus(fileSelect.value);
        await mf4LoadSignalsForSelectedFile();
    });

    if (rawFilesSelect) rawFilesSelect.addEventListener('change', () => {
        mf4HandleViewerFileMenuChange(rawFilesSelect).catch((e) => {
            const message = (e && typeof e === 'object' && 'message' in e) ? String(e.message || e) : String(e);
            mf4SetStatus(message, true);
        });
    });

    if (exportedFilesSelect) exportedFilesSelect.addEventListener('change', () => {
        mf4HandleViewerFileMenuChange(exportedFilesSelect).catch((e) => {
            const message = (e && typeof e === 'object' && 'message' in e) ? String(e.message || e) : String(e);
            mf4SetStatus(message, true);
        });
    });

    // ── Expandable file-picker toggle buttons ──
    document.querySelectorAll('.mf4-file-picker-toggle').forEach((btn) => {
        btn.addEventListener('click', () => {
            const picker = btn.closest('.mf4-file-picker');
            if (!picker) return;
            const isOpen = picker.classList.toggle('open');
            btn.setAttribute('aria-expanded', String(isOpen));
            // Close the other picker
            document.querySelectorAll('.mf4-file-picker').forEach((other) => {
                if (other !== picker && other.classList.contains('open')) {
                    other.classList.remove('open');
                    const otherBtn = other.querySelector('.mf4-file-picker-toggle');
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.mf4-file-picker')) {
            document.querySelectorAll('.mf4-file-picker.open').forEach((p) => {
                p.classList.remove('open');
                const b = p.querySelector('.mf4-file-picker-toggle');
    ...
    if (signalsSelect) signalsSelect.addEventListener('change', () => {
        ...
        if (exportBtn) exportBtn.disabled = !mf4ShouldEnableDecodedExport();
    });
...
async function mf4ExportDecodedMf4() {
    ...
    if (!mf4ShouldEnableDecodedExport()) {
        mf4SetStatus('Export coded MF4 e disponibile solo per file raw non decodificati.', true);
        return;
    }
    ...
    const payload = { file };
    if (signals.length > 0 || ch !== null || start_s !== null || end_s !== null) {
        payload.respect_view_selection = true;
        if (signals.length > 0) payload.signals = signals;
        if (ch !== null) payload.channel = ch;
        if (start_s !== null) payload.start_s = start_s;
        if (end_s !== null) payload.end_s = end_s;
    }

    mf4SetStatus('Exporting coded MF4 with the active live decoder...');
    ...
    try {
        ...
        if (!r.ok || !result.ok) {
            throw new Error(result?.error ? String(result.error) : `HTTP ${r.status} ${r.statusText}`);
        }

        const name = String(result.file || 'coded.mf4').trim();
        const relativeName = String(result.relative_name || (name ? `exports/${name}` : '')).trim();
        await mf4LoadFiles(relativeName || undefined);
        loadLogs().catch(() => {});
        mf4SetStatus(relativeName ? `Opened exported MF4: ${name}` : `Saved to logs/exports: ${name}`);
    } catch (e) {
        console.error(e);
        const message = (e && typeof e === 'object' && 'message' in e) ? String(e.message || e) : String(e);
        mf4SetStatus(`Export error: ${message}`, true);
    } finally {
        if (exportBtn) exportBtn.disabled = !mf4ShouldEnableDecodedExport();
    }
}
...
function mf4IsDecodedSelectionMode() {
    const wrap = document.getElementById('mf4-messages-wrap');
    if (!wrap) return false;
    return wrap.style.display !== 'none' && Array.isArray(mf4DecodedGroupsCache) && mf4DecodedGroupsCache.length > 0;
}

function mf4ShouldEnableDecodedExport() {
    return !!mf4RawExportAvailable;
}
...
async function mf4LoadDbcs() {
    ...
    try {
        // Fetch all database types in parallel: DBC, FIBEX, ARXML.
        const [dbcs, fibexs, arxmls] = await Promise.all([
            mf4FetchJson('/api/dbcs').catch(() => []),
            mf4FetchJson('/api/fibexs').catch(() => []),
            mf4FetchJson('/api/arxml/files').catch(() => ({ ok: true, files: [] })),
        ]);
        ...
        // DBC files
        const dbcList = Array.isArray(dbcs) ? dbcs : [];
        if (dbcList.length) {
            ...
            dbcSelect.appendChild(grp);
        }

        // ARXML files
        const arxmlFiles = Array.isArray(arxmls?.files) ? arxmls.files : (Array.isArray(arxmls) ? arxmls : []);
        if (arxmlFiles.length) {
            const grp = document.createElement('optgroup');
            grp.label = 'ARXML (CAN/FlexRay)';
            arxmlFiles.forEach((d) => {
                const name = String(d?.name || d || '').trim();
                if (!name) return;
                const opt = document.createElement('option');
                opt.value = name;
    ...
    try {
        ...
        if (items.length > 0) {
            items.forEach((item) => {
                ...
                if (!v) return;
                const label = String(item?.label || `CH ${v}`).trim();
                const bestDbc = String(item?.best_dbc || '').trim();
                const opt = document.createElement('option');
                opt.value = v;
                opt.text = bestDbc ? `${label} [auto ${bestDbc}]` : label;
                if (bestDbc) opt.title = `Suggested decode: ${bestDbc}`;
        ...
        } else if ('channel' in st) {
            // User explicitly chose "All channels" (channel='') — respect it.
            chSelect.value = '';
        } else {
            const { auto, dbcs } = mf4GetSelectedDbcs();
            const suggested = String(res?.suggested_channel ?? '').trim();
            if (auto && dbcs.length === 0 && suggested !== '' && Array.from(chSelect.options).some(o => o.value === suggested)) {
                chSelect.value = suggested;
            } else {
                chSelect.value = '';
            }
        }
...
async function mf4LoadFiles(preferredFile) {
    const fileSelect = document.getElementById('mf4-file-select');
    const rawFilesSelect = document.getElementById('mf4-raw-files-select');
    const exportedFilesSelect = document.getElementById('mf4-exported-files-select');
    const signalsSelect = document.getElementById('mf4-signals-select');
    ...
    try {
        const files = await mf4FetchJson('/api/mf4/files?include_exports=1');
        const rows = Array.isArray(files) ? files : (files.files || []);
        const entries = (rows || []).map((row) => {
            if (row && typeof row === 'object') {
                const name = String(row.name || '').trim();
                const basename = String(row.basename || '').trim() || (name.split('/').pop() || name);
                const bucket = String(row.bucket || (name.startsWith('exports/') ? 'exports' : 'logs')).trim() || 'logs';
                return { ...row, name, basename, bucket };
            }
            const name = String(row || '').trim();
            const basename = name.split('/').pop() || name;
            return { name, basename, bucket: name.startsWith('exports/') ? 'exports' : 'logs', size: 0 };
        }).filter(entry => String(entry?.name || '').trim());

        mf4FilesCache = entries;
        ...
        if (entries.length === 0) {
            ...
            mf4SetFileMenuPlaceholder(rawFilesSelect, 'No raw MF4 files found');
            mf4SetFileMenuPlaceholder(exportedFilesSelect, 'No exported MF4 files found');
        ...
        const logEntries = entries.filter(entry => entry.bucket !== 'exports');
        const exportEntries = entries.filter(entry => entry.bucket === 'exports');
        const appendOptions = (parent, items) => {
            items.forEach((entry) => {
                ...
                const opt = document.createElement('option');
                opt.value = name;
                opt.text = String(entry?.basename || name);
                parent.appendChild(opt);
            });
        };

        appendOptions(fileSelect, logEntries);
        appendOptions(fileSelect, exportEntries);
        mf4PopulateFileMenu(rawFilesSelect, logEntries, 'No raw MF4 files found');
        mf4PopulateFileMenu(exportedFilesSelect, exportEntries, 'No exported MF4 files found');

        // Restore last selected file if present
        const st = mf4LoadState();
        const lastFile = String((preferredFile || st.file) || '').trim();
        if (lastFile && Array.from(fileSelect.options).some(o => o.value === lastFile)) {
            fileSelect.value = lastFile;
            mf4SyncViewerFileMenus(lastFile);
            await mf4LoadSignalsForSelectedFile();
        } else {
            mf4SyncViewerFileMenus('');
        }

        if (!fileSelect.value) {
            mf4SetStatus('Select an MF4 file');
        }
    } catch (e) {
        console.error(e);
        mf4SetFileMenuPlaceholder(rawFilesSelect, 'Unable to load raw MF4 files');
        mf4SetFileMenuPlaceholder(exportedFilesSelect, 'Unable to load exported MF4 files');
...
async function mf4LoadSignalsForSelectedFile() {
    ...
    const isRawCan = (info && info.ok && String(info.kind) === 'raw_can');
    const hasDecodedChannels = !!(info && info.ok && info.has_decoded_channels);
    mf4RawExportAvailable = !!isRawCan;

    // When file already contains decoded channels and the user hasn't explicitly
    // selected a database, prefer the embedded channels directly.
    const { auto, dbcs } = mf4GetSelectedDbcs();
    const useEmbeddedDecoded = hasDecodedChannels && dbcs.length === 0;

    if (dbcSelect) dbcSelect.disabled = !isRawCan;
    if (chSelect) chSelect.disabled = !(isRawCan && !useEmbeddedDecoded);
    ...
    signalsSelect.innerHTML = '';
    if (useEmbeddedDecoded) {
        mf4SetStatus('MF4 con segnali decodificati integrati. Il DBC e opzionale e serve solo per ri-decodificare dal raw.');
    } else if (info && info.ok && !isRawCan) {
    ...
    try {
        if (useEmbeddedDecoded) {
            // Show embedded decoded channels directly from the file (no DBC needed).
            const decodedNames = Array.isArray(info.decoded_channel_names) ? info.decoded_channel_names : [];
            if (decodedNames.length === 0) {
                mf4SetStatus('No decoded channels found in file', true);
            ...
            if (exportBtn) exportBtn.disabled = !mf4ShouldEnableDecodedExport();
            if (selectAllBtn) selectAllBtn.disabled = !(signalsSelect && signalsSelect.options && signalsSelect.options.length > 0);
            mf4SetStatus(`${decodedNames.length} segnali decodificati integrati. Seleziona e plotta. Export coded MF4 usa sempre il raw live-decoded.`);
            return;
        } else if (isRawCan && (dbcs.length > 0 || auto)) {
            const qs = new URLSearchParams();
            qs.set('file', file);
            ...
            if (decoded.groups.length === 0) {
                mf4SetStatus('No DBC messages found in this MF4 (try a different DBC or log file)', true);
                mf4UpdateSignalCount(0, 0);
                return;
            }
            ...
            mf4SetStatus('Select one or more signals. Export coded MF4 ignores viewer selections and uses the active live decoder.');
            return;
        } else {
...
async function mf4FetchJson(url, options) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Retry on network-level errors (TypeError: Failed to fetch)
...
async function mf4PlotSelectedSignals() {
    ...
    try {
        ...
        const hasDecodedChannels = !!(info && info.ok && info.has_decoded_channels);
        const useEmbeddedDecoded = hasDecodedChannels && dbcs.length === 0;
        const useDecoded = isRawCan && !useEmbeddedDecoded && (dbcs.length > 0 || auto) && signals.some(s => String(s).includes('.'));
        const url = useDecoded ? '/api/mf4/decoded_data' : '/api/mf4/data';
        const body = useDecoded
            ? { file, dbcs, auto: !!auto, channel: (ch === null ? null : ch), signals, start_s, end_s, max_points }
...
async function runScanTools(action) {
    // Actions that talk to the vehicle over DoIP (Automotive Ethernet) and
    // therefore don't need a CAN channel selected in the ScanTools panel.
    const DOIP_ONLY_ACTIONS = new Set([
    ...
    // Actions that prefer CAN but transparently fall back to DoIP server-side
    // when no CAN channel is active. The frontend must not block them when the
    // ScanTools channel select is empty (e.g. user hasn't configured a row in
    // Channel Configuration yet but DoIP is reachable).
    const CHANNEL_OPTIONAL_ACTIONS = new Set([
        'clear_dtcs',
    ]);

    let channelId = getSelectedChannelId();
    let usingDoipFallback = false;
    if (channelId === null) {
        if (DOIP_ONLY_ACTIONS.has(action) || CHANNEL_OPTIONAL_ACTIONS.has(action)) {
            channelId = 0;
            usingDoipFallback = CHANNEL_OPTIONAL_ACTIONS.has(action);
        } else {
    ...
    if (scanToolsStatus) {
        const noChan = DOIP_ONLY_ACTIONS.has(action) || usingDoipFallback;
        const extra = noChan ? '' : ` (channel ${channelId})`;
    ...
    try {
        ...
        // Don't auto-start the Bus System for actions that are running via the
        // DoIP fallback path (no CAN channel selected) — the backend handles
        // that case directly and starting CAN here would be wrong.
        if (!DOIP_ONLY_ACTIONS.has(action) && !usingDoipFallback
            && !(res.ok && data.status === 'started') && _isChannelNotActiveError(data)) {
            appendScanLog('Channel not active. Auto-starting Bus System…');
            await _ensureBusStartedForSelectedChannel(channelId);
...
async function loadLogs() {
    const container = document.getElementById('log-files-list');
    const exportedContainer = document.getElementById('exported-log-files-list');
    const homeExportedContainer = document.getElementById('home-exported-files-list');
    if (container) container.innerHTML = '<div class="list-group-item text-center text-muted">Loading...</div>';
    if (exportedContainer) exportedContainer.innerHTML = '<div class="list-group-item text-center text-muted">Loading...</div>';
    if (homeExportedContainer) homeExportedContainer.innerHTML = '<div class="list-group-item text-center text-muted">Loading...</div>';

    const renderLogList = (target, files, emptyText, allowDecode) => {
        if (!target) return;
        target.innerHTML = '';
        const rows = Array.isArray(files) ? files : [];
        if (rows.length === 0) {
            target.innerHTML = `<div class="list-group-item text-center text-muted">${emptyText}</div>`;
            return;
        }
        ...
        rows.forEach((f) => {
            ...
            if (!name) return;
            const displayName = String(f?.basename || name.split('/').pop() || name).trim();
            const sizeBytes = (f?.size_total != null) ? f.size_total : f.size;
            const sizeStr = mf4FormatFileSize(sizeBytes);
            const isMf4 = displayName.toLowerCase().endsWith('.mf4');
            const isTmpMf4 = displayName.toLowerCase().endsWith('.tmp.mf4');
            ...
            item.innerHTML = `
                <div class="text-truncate me-2" title="${displayName}">
                    <small class="fw-bold d-block">${displayName}</small>
                    <small class="text-muted">${sizeStr}${partSuffix}</small>
                </div>
                <div class="d-flex gap-2">
                    ${allowDecode && isMf4 && !isTmpMf4 ? `
                    <button type="button" class="btn btn-sm btn-outline-secondary btn-mf4-decode" data-filename="${encodeURIComponent(name)}" title="Decode MF4 to CSV (offline)">
                        CSV
                    </button>
                    ` : ''}
                    <a href="${mf4BuildLogHref(name)}" class="btn btn-sm btn-outline-primary" download="${displayName}" title="Download">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16">
                            <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/>
                            <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"/>
                        </svg>
                    </a>
                    <button type="button" class="btn btn-sm btn-outline-danger btn-log-delete" data-filename="${encodeURIComponent(name)}" title="Delete">
    ...
    const bindDeleteButtons = (target) => {
        if (!target) return;
        target.querySelectorAll('.btn-log-delete').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                try {
                    const deleteUrl = mf4BuildLogHref(name);
                    ...
                    if (!resp.ok) {
                        if (resp.status === 409) {
                            ...
                            const retry = await fetch(deleteUrl, { method: 'DELETE' });
                            if (!retry.ok) {
                                const txt2 = await retry.text();
    ...
    const bindDecodeButtons = (target) => {
        if (!target) return;
        target.querySelectorAll('.btn-mf4-decode').forEach(btn => {
            btn.addEventListener('click', async () => {
                const encName = btn.getAttribute('data-filename');
                const name = decodeURIComponent(encName || '');
                if (!name) return;
    ...
    try {
        const [logsResult, exportedResult] = await Promise.allSettled([
            fetch('/api/logs').then(async (res) => {
                if (!res.ok) throw new Error(`HTTP ${res.status}`);
                return res.json();
            }),
            fetch('/api/mf4/files?include_exports=1').then(async (res) => {
                if (!res.ok) throw new Error(`HTTP ${res.status}`);
                return res.json();
            }),
        ]);
        ...
        if (logsResult.status === 'fulfilled') {
            renderLogList(container, logsResult.value, 'No logs found', true);
            bindDeleteButtons(container);
            bindDecodeButtons(container);
        } else if (container) {
            container.innerHTML = '<div class="list-group-item text-center text-danger">Error loading logs</div>';
        }

        if (exportedResult.status === 'fulfilled') {
            const mf4Rows = Array.isArray(exportedResult.value) ? exportedResult.value : (exportedResult.value?.files || []);
            const exportedFiles = mf4Rows
                .map((row) => {
                    if (!row || typeof row !== 'object') return null;
                    const name = String(row.name || '').trim();
                    if (!name || String(row.bucket || '').trim() !== 'exports') return null;
                    return {
                        ...row,
                        name,
                        basename: String(row.basename || '').trim() || (name.split('/').pop() || name),
                    };
                })
                .filter(Boolean);
            renderLogList(exportedContainer, exportedFiles, 'No exported files found', false);
            bindDeleteButtons(exportedContainer);
            renderLogList(homeExportedContainer, exportedFiles, 'No exported files found', false);
            bindDeleteButtons(homeExportedContainer);
        } else if (exportedContainer) {
            exportedContainer.innerHTML = '<div class="list-group-item text-center text-danger">Error loading exported files</div>';
            if (homeExportedContainer) homeExportedContainer.innerHTML = '<div class="list-group-item text-center text-danger">Error loading exported files</div>';
        }
    } catch (e) {
        console.error(e);
        if (container) {
        ...
        if (homeExportedContainer) {
            homeExportedContainer.innerHTML = '<div class="list-group-item text-center text-danger">Error loading exported files</div>';
        }
    }
}
...
// Limite righe della tabella Live Traffic. Configurabile dal browser via
// localStorage.setItem('kbsm_log_limit', '5000'); persiste finché il tab è
// aperto. Default 2000: con throttle UI 1 Hz + dedupe per (ch, arb_id),
// 2000 righe coprono ~2 secondi di update completi (CAN/FR/LIN tutti
// rappresentati), evitando che i batch grandi facciano "scomparire" i
// frame meno frequenti (es. FlexRay) sotto la marea dei CAN.
function trimLogTable(limit = null) {
    if (!logTable) return;
    if (limit === null) {
        const stored = parseInt(window.localStorage?.getItem('kbsm_log_limit') || '', 10);
        limit = (Number.isFinite(stored) && stored > 0) ? stored : 2000;
    }
    while (logTable.children.length > limit) {
        const last = logTable.lastElementChild;
        if (!last) break;
        last.remove();
    }
}

function buildEthLogRow(pkt) {
...
function renderEthPackets(packets) {
    if (!logTable) return;
    ...
    for (let i = list.length - 1; i >= 0; i -= 1) {
        frag.appendChild(buildEthLogRow(list[i]));
    }
    logTable.prepend(frag);
    trimLogTable();
}

socket.on('eth_packet', (pkt) => {
    renderEthPackets(pkt);
});

socket.on('eth_packet_batch', (packets) => {
    renderEthPackets(packets);
});
...
function buildBusLogRow(frame) {
    ...
    const origin = String(frame?.capture_origin || '').trim().toLowerCase();
    const type = String(frame?.type || '').trim().toUpperCase();
    let inputLabel = '—';
    if (frame?.channel != null && String(frame.channel).trim() !== '') {
        inputLabel = String(frame.channel);
    }
    if (type === 'FLEXRAY' || type === 'FLEX' || type === 'FR') {
        inputLabel = `FR ${inputLabel}`;
    } else if (type === 'LIN') {
        inputLabel = `LIN ${inputLabel}`;
    } else if (type === 'ETH' || type === 'ETHERNET') {
        inputLabel = 'ETH';
    } else {
        inputLabel = `CAN ${inputLabel}`;
    }
    if (origin === 'mirror') {
        inputLabel += ' mirror';
    }
...
function handleBusFrames(frames) {
    ...
    const frag = document.createDocumentFragment();
    for (let i = list.length - 1; i >= 0; i -= 1) {
        frag.appendChild(buildBusLogRow(list[i]));
    }
    logTable.prepend(frag);
    trimLogTable();
}

socket.on('timeline_bus_data', (frame) => {
    const list = Array.isArray(frame) ? frame : [frame];
    for (const item of list) {
        try {
            timelineOnBusFrame(item);
        } catch (e) {
            // ignore
        }
    }
...
socket.on('timeline_bus_data_batch', (frames) => {
    const list = Array.isArray(frames) ? frames : [frames];
    for (const item of list) {
        try {
            timelineOnBusFrame(item);
...
socket.on('bus_data', (frame) => {
    handleBusFrames(frame);
});

socket.on('bus_data_batch', (frames) => {
    handleBusFrames(frames);
});
